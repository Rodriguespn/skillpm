...
export default async function HomePage() {
  const entities = await Promise.all(
    ENTITIES.map(async (entity) => {
      ...
      return { entity, index, versionsIndex }
    })
  )
  ...
  return (
    <div>
      ...
      <div className="mb-10 rounded-xl border border-zinc-200 bg-white p-5 text-sm">
        <p className="font-medium text-zinc-700 mb-3">How it works</p>
        <div className="flex flex-col gap-2 text-zinc-500">
          <Row n="1" text="A publisher serves skills at" code="/.well-known/agent-skills/index.json" />
          <Row n="2" text="Versioned snapshots are immutable at" code="/v0.1.1/index.json" />
          <Row n="3" text="Per-skill version history lives at" code="/{skill}/versions.json" />
          <Row n="4" text="This registry reads those endpoints and visualises them" />
        </div>
      </div>
      ...
      <div className="space-y-4">
        {entities.map(({ entity, index, versionsIndex }) => (
          <Link
            key={entity.id}
            href={`/${entity.id}`}
          ...
          >
            ...
            <div className="mt-4 flex items-center gap-5 text-xs text-zinc-400 border-t border-zinc-50 pt-4">
              ...
              <span><strong className="text-zinc-600">{versionsIndex.versions.length}</strong> releases</span>
...
function Row({ n, text, code }: { n: string; text: string; code?: string }) {
  return (
    <div className="flex items-baseline gap-2">
      ...
      <span>{text}{code && <> <code className="font-mono text-xs bg-zinc-100 px-1 py-0.5 rounded text-zinc-600">{code}</code></>}</span>
    </div>
  )
}
